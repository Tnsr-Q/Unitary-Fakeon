Below is a complete, mathematically rigorous construction of the **Elliptic Principal-Value (PV) Integration Rules** for the fakeon prescription. The framework maps the fakeon's purely virtual condition onto the real homology of a genus-1 curve, defines PV action on iterated elliptic integrals and modular forms, and proves that the imaginary-period cancellation mechanism survives the transition from logarithmic branch cuts to elliptic branch points. This delivers a systematic, real-analytic calculus for elliptic sectors that preserves perturbative unitarity without spurious imaginary periods.
